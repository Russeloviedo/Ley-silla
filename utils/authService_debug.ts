...
export class AuthService {
  ...
  static async verifyUserBasic(rfc: string, nombre: string): Promise<AuthResult> {
    ...
    if (!user) {
      console.log('❌ RFC no encontrado en la base de datos');
      return {
        success: false,
    ...
    if (user.nombre !== nombre) {
      console.log('❌ Nombre no coincide: DB:', user.nombre, 'vs Recibido:', nombre);
      return {
        success: false,
    ...
    if (existingUser) {
      console.log('🔄 Usuario ya existe, verificando dispositivo...');
      // Verificar si es el mismo dispositivo
      const isSameDevice = await DeviceService.isSameDevice(existingUser.deviceId);
      console.log('📱 Es mismo dispositivo:', isSameDevice);
      
      if (isSameDevice) {
        console.log('✅ Mismo dispositivo, acceso directo permitido');
        ...
        console.log('🔄 Dispositivo diferente, requiere segunda verificación');
    ...
    console.log('🆕 Usuario nuevo, creando datos...');
  ...
  static async getUserByRFC(rfc: string): Promise<UserData | null> {
    try {
      console.log('🔍 Buscando usuario por RFC:', rfc);
